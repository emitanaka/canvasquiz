...
#' @param course_id The ID of the Canvas course where the file is stored (default is taken from the environment variable `CANVASQUIZ_COURSE_ID`).
#' @return A character string containing the HTML `<img>` tag for the specified image file.
#' @export
tag_img <- function(file_id, width = "100%", class = "", course_id = Sys.getenv("CANVASQUIZ_COURSE_ID")) {
...
#' @rdname tag_file
#' @export
upload_tag_file <- function(file_path, text = "Download", folder_id = NULL, course_id = Sys.getenv("CANVASQUIZ_COURSE_ID"), url = Sys.getenv("CANVASQUIZ_URL"), token = Sys.getenv("CANVASQUIZ_TOKEN")) {
...
#' @rdname tag_img
#' @export
upload_tag_img <- function(file_path, width = "100%", class = "", folder_id = NULL, course_id = Sys.getenv("CANVASQUIZ_COURSE_ID"), url = Sys.getenv("CANVASQUIZ_URL"), token = Sys.getenv("CANVASQUIZ_TOKEN")) {
  fid <- upload_file(file_path, folder_id, course_id, url, token)
